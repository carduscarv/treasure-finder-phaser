...
export class PhaserUIComponent {
     ...
     */
    Button(scene, x, y, config) {
        const { type, texture, label, callback, arg, style, callbackOnUp, onToggleTexture, isToggleActive, spritesheetTexture } = config;
        let btn = null;
        switch (type) {
            case 'Radio':
     ...
     */
    VirtualControl(scene, x, y, config) {
        const { type, spritesheetTexture, callback, argument, width, height, arrowConfig } = config;
     ...
     */
    TextField(scene, x, y, config, style) {
        const { type } = config;
        let textField = null;
        switch (type) {
            case 'Flat':
